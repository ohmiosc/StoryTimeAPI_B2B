const fetch = require('node-fetch');
const google = require('googleapis');
const pub = google.androidpublisher('v2');
import * as constants from '../constants';

export class GoogleAPI {

  private readonly authClient;

  constructor() {

    this.authClient = new google.auth.JWT(
      constants.GOOGLE_AUTH_CLIENT_EMAIL,
      '',
      constants.GOOGLE_AUTH_PRIVATE_KEY,
      [
        'https://www.googleapis.com/auth/androidpublisher',
      ],
      '',
    );

  }

  private getReceipt = (params) => new Promise((resolve, reject) => {
    pub.purchases.subscriptions.get(params, (error, resp) => {
      error ? reject(JSON.stringify(error.response.data)) : resolve(resp.data);
    });
  })

  private async authorize() {
    console.log('Email = ', constants.GOOGLE_AUTH_CLIENT_EMAIL);
    console.log('Key = ', constants.GOOGLE_AUTH_PRIVATE_KEY);

    try {
      await this.authClient.authorize();
      return true;
    } catch (e) {
      console.log(`Authorization failed`, e.toString());
      return false;
    }
  }

  public async getGoogleReceipt(inputPackageName: string, inputSubscriptionId: string, purchaseToken: string): Promise<any> {
    console.log(`Getting receipt info from Google Play for id ${inputSubscriptionId} and payload ${purchaseToken}`);

    try {
      const isAuthorized = await this.authorize();

      if (!isAuthorized) {
        console.log(`Request is not authorized`);
        return false;
      }

      const params = {
        auth: this.authClient,
        packageName: inputPackageName,
        subscriptionId: inputSubscriptionId,
        token: purchaseToken,
      };

      const receipt: any = await this.getReceipt(params);
      console.log('Google receipt = ', JSON.stringify(receipt));

      return receipt;

    } catch (e) {
      console.log(`An error has occurred while getting response from Google Play ${e.toString()}`);
      return null;
    }
  }

  async getGoogleReceiptV3(inputPackageName, inputSubscriptionId, purchaseToken) {
    console.log(`Getting receipt info from Google Play for id ${inputSubscriptionId} and payload ${purchaseToken}`);

    let authorizationResponse;
    try {
      authorizationResponse = await this.authClient.authorize();
    } catch (e) {
      console.error('Authorization Error', e.toString());
      return undefined;
    }

    if (!authorizationResponse || !authorizationResponse.access_token) {
      console.error('Authorization FAILED');
      return undefined;
    }

    try {
      const url = `https://www.googleapis.com/androidpublisher/v3/applications/${inputPackageName}/purchases/subscriptions/${inputSubscriptionId}/tokens/${purchaseToken}?access_token=${authorizationResponse.access_token}`;
      const response = await fetch(url, { method: 'GET' });

      if (!response) {
        console.error('Error occurred while fetching google receipt. Fetch response is undefined');
        return undefined;
      }

      return await response.json();

    } catch (e) {
      console.error('Error occurred while fetching google receipt', e.toString());
      return undefined;
    }

  }

}
